Реализация библиотеки `s21_containers.h` для создания универсальных контейнеров в C++, включая списки, карты, очереди, множества, стеки и векторы.

## Оглавление

1. [Введение](#введение)
2. [Общая информация](#общая-информация)
3. [Реализация библиотеки](#реализация-библиотеки)
    - [Часть 1: Основные контейнеры](#часть-1-основные-контейнеры)
    - [Часть 2: Дополнительные контейнеры](#часть-2-дополнительные-контейнеры)
    - [Часть 3: Методы insert_many](#часть-3-методы-insert_many)

## Введение

В рамках проекта предлагается разработать собственную библиотеку, реализующую стандартные контейнерные классы языка C++: `list`, `map`, `queue`, `set`, `stack`, и `vector`. Эти реализации должны предоставлять полный набор стандартных методов и атрибутов для работы с элементами и итерирования.

## Общая информация

Контейнеры — это структуры данных, которые организуют наборы объектов одного типа. Они предоставляют различные методы для эффективного управления данными, включая доступ к элементам, проверку заполненности и итерирование.

### Методы контейнеров

Каждый вид контейнеров предоставляет:
- Конструкторы (стандартные и с параметрами)
- Доступ к элементам (например, по индексу)
- Проверку на пустоту и получение размера
...
Необходимо реализовать основные классы контейнеров, предоставляя шаблонные реализации для различных типов данных. Примеры классов: `list`, `map`, `queue`, `set`, `stack`, `vector`.

### Часть 2: Дополнительные контейнеры

Разработка дополнительных контейнеров, таких как `array` и `multiset`, которые расширяют базовую функциональность стандартных контейнеров.

### Часть 3: Методы insert_many

Реализация методов для вставки множественных элементов в контейнеры. Эти методы должны позволять вставку нескольких элементов одновременно в различные типы контейнеров.

## Сборка и тестирование

Используйте `Makefile` для сборки библиотеки и запуска тестов:
